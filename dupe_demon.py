...
PLUGIN_STATE_FILE = APP_SUPPORT_DIR / "plugin_state.json"
...
# .py file dropped into ~/Library/Application Support/DupeDemon/plugins/
...
def _load_plugin_state():
    ...
    try:
        return json.loads(PLUGIN_STATE_FILE.read_text())
        ...
        for path in sorted(d.glob("*.py")):
            ...
            try:
                spec = importlib.util.spec_from_file_location(
                    f"dupedemon_plugin_{path.stem}", path)
...
def _auto_pool_size() -> int:
    """Thread count for a scan-phase pool, sized from this Mac's CPU and
    memory rather than a user-tunable slider (a maxed-out manual setting
    used to ask the OS for over a thousand threads and the scheduling
    overhead alone made the app look hung on real libraries)."""
    ...
    if mem_gb < 4:
        ...
        workers = cores * 4
    return max(4, min(workers, 64))


_POOL_SIZE = _auto_pool_size()


def _pool_size(prefs=None) -> int:
    return _POOL_SIZE
...
class Preferences:
    ...
    protected_folder_consent: Optional[bool] = None  # scans of Desktop/Documents/Downloads/
                                            # Pictures/Movies/Music/volumes: None=never asked
...
def find_exact_duplicates(files, prefs, progress=None, cancel_event=None, cache=None,
    ...
    groups = []
    if status:
        status(f"Comparing {total} files of matching size…" if total
               else "No same-size files to compare.")
    ...
    if cache is not None:
        ...
        new_entries = []
        with ThreadPoolExecutor(max_workers=_pool_size(prefs)) as pool:
...
def _fill_dimensions(groups, prefs, status=None):
    """Read image dimensions for grouped files (exact mode skips decoding)."""
    if status:
        status("Reading image details…")
...
def run_scan(folders, prefs, progress=None, status=None, cancel_event=None):
    ...
    try:
        if prefs.match_mode == "exact":
            ...
            _fill_dimensions(groups, prefs, status=status)
        else:
...
def _resource_dir() -> Path:
    """Bundled-assets root: PyInstaller's unpack dir when frozen (set via
    sys._MEIPASS in both onefile and onedir builds), else the source
    checkout next to this file."""
...
def protected_folder_category(path):
    """Name of the TCC-gated category `path` falls under, or None.

    Covers the per-folder categories macOS gates for unsandboxed apps
    (Desktop/Documents/Downloads/Pictures/Movies/Music, plus anything on a
    mounted volume). Confirmed by direct testing that a "Don't Allow" click
    on the OS prompt does not reliably stop a later os.walk/os.listdir from
    an unsandboxed, non-Powerbox call — so Dupe Demon enforces a decline
    itself rather than trusting the OS to.
    """
    try:
        resolved = Path(path).resolve()
    except OSError:
        return None
    for name, folder in PROTECTED_FOLDER_CATEGORIES.items():
        try:
...
def build_gui():
    ...
    class PieProgress(ProgressIndicator):
        """Same value/maximum-driven start/stop API every call site already
        uses — backed by the shared cooliosis spinner instead of hand-drawn
        Canvas pie-slice math. `value`/`maximum` no longer paint a fill
        fraction (a plain frame loop has no percentage concept), but every
        call site already mirrors its own "N of M" detail into
        status_label as text, so no progress information is actually lost."""
        _FRAMES = None  # loaded once, shared by the single instance

        def __init__(self, master, size=28, bg_color=None, fg_color=None, **kw):
            if PieProgress._FRAMES is None:
                PieProgress._FRAMES = sequence_frames(
                    _resource_dir() / "assets" / "progress_frames", size=size)
            super().__init__(master, PieProgress._FRAMES, fps=50, **kw)
            self._value = 0
            self._maximum = 100
        ...
        def config(self, **kw):
            ...
            if kw:
                super().config(**kw)
            if changed:
                # The old Canvas pie used value==maximum ("100%") as its own
                # "done" signal, relying on a coincidence that doesn't carry
                # over: a fully-filled pie renders as a solid circle, which
                # looks identical whether "spinning" or not, so callers could
                # get away with never explicitly stopping it at completion.
                # A real playing animation has no such disguise — it visibly
                # loops forever unless told to stop, so reaching/exceeding
                # maximum has to mean "stop", not "start".
        ...
        def stop_spinning(self):
            """Freeze on the current frame without resetting — matches the
            old pie's "leave it showing where it stopped" behavior for a
            caller that stops mid-render (e.g. "first N of M") without
            explicitly zeroing value."""
    ...
    class PreferencesWindow(tk.Toplevel):
        def __init__(self, master, prefs: Preferences, on_save):
            ...
            # --- Privacy ----------------------------------------------------
            ttk.Label(body, text="Privacy", font=("", 13, "bold")).grid(
                row=row, column=0, columnspan=3, sticky="w"); row += 1

            self._protected_initial = bool(prefs.protected_folder_consent)
            self.protected_var = tk.BooleanVar(value=self._protected_initial)
            ttk.Checkbutton(
                body, text="Allow scanning Desktop, Documents, Downloads, "
                           "Pictures, Movies, Music, and external/network volumes",
                variable=self.protected_var).grid(
                row=row, column=0, columnspan=3, sticky="w", pady=2); row += 1
            ttk.Label(body, foreground="gray", wraplength=460, justify="left", text=(
                "When off, scans skip these folders even if one is in the "
                "list — Dupe Demon asks the first time a scan needs one.")
        ...
        def _restore_defaults(self):
            ...
            self.protected_var.set(bool(defaults.protected_folder_consent))
            self._update_state()
        ...
        def _save(self):
            ...
            if self.protected_var.get() != self._protected_initial:
                # Only write when the box was actually touched — leaves an
                # untouched "never asked" (None) as None rather than
                # collapsing it to False on every unrelated Preferences save.
                p.protected_folder_consent = self.protected_var.get()
            p.save()
            self.on_save()
            self.destroy()
    ...
    class App(_TkBase):
        def __init__(self):
            ...
            applog.info(f"{APP_NAME} v{__version__} started.")

        # Tk's default report_callback_exception just prints to stderr, which
        # is invisible once this is a packaged .app — nothing is left to
        # diagnose a bug that wasn't already wrapped in an explicit try/except.
        ...
        def _build_menubar(self):
            ...
            plugins_menu.add_command(label="Manage Plugins…",
                                      command=self._open_plugin_manager)
            plugins_menu.add_command(label="Open Plugins Folder…",
            ...
            help_menu.add_separator()
            help_menu.add_command(
                label="View Log in Console...",
                command=lambda: self._open_log_console())
            help_menu.add_command(
                label="Reveal Log in Finder",
                command=lambda: self._reveal_log())
        ...
        def _build_widgets(self):
            ...
            ttk.Button(actions, text="Clear Selection", command=self.clear_selection).pack(
                side="left", padx=(6, 0))
            self.trash_btn = ttk.Button(actions, text="🗑 Delete",
                                        command=self.trash_selected)
        ...
        # ---------------- folder management ----------------
        def add_folder(self):
            # Tk's directory picker has no native multi-select on any
            # platform — it can only return one folder per call. Finder's
            # own window DOES support real Cmd/Shift-click multi-select, and
            # this app already accepts multi-folder drops, so just open
            # Finder there (same plain `open` pattern as reveal_in_finder /
            # open_file elsewhere in this file) and let the user drag
            # several folders onto the list at once. Forcing a window size
            # via AppleScript `set bounds` doesn't reliably stick — Finder
            # keeps using whatever size it last remembered — so don't fight
            # it; a window the user already resized themselves is normal
            # Finder behavior, not a bug.
            if self.folders:
                start = os.path.dirname(self.folders[-1]["path"])
            else:
                pictures = str(Path.home() / "Pictures")
                start = pictures if os.path.isdir(pictures) else str(Path.home())
            subprocess.run(["open", start])
            self.status_label.config(
                text="Select one or more folders in the Finder window "
                     "(⌘-click for multiple), then drag them onto the "
        ...
        def clear_cache_action(self):
            # Button is disabled whenever the cache is empty (see
            # refresh_cache_button), so reaching here means there's real
            # work to do. No confirmation dialog — clearing the hash cache
            # is fully safe (it only makes the next scan re-hash files;
            # nothing in the library is touched), and a hidden modal here
            # was the reason clicking used to look like it did nothing.
            ok = clear_hash_cache()
            self.refresh_cache_button()
            self.status_label.config(
        ...
        # ---------------- scanning ----------------
        def _filter_consented_folders(self, folders):
            """Return (folders cleared to scan, status note or None).

            Splits out any folder under a TCC-gated category (see
            protected_folder_category) so a decline is enforced by the app
            itself, regardless of whether macOS's own prompt does. Consent
            is asked once and persisted to prefs.protected_folder_consent
            (None=never asked); Preferences also has a checkbox to flip it
            later without waiting for a scan.
            """
            tagged = [(e, protected_folder_category(e["path"])) for e in folders]
            protected = [(e, cat) for e, cat in tagged if cat]
            if not protected:
                return folders, None

            if self.prefs.protected_folder_consent is None:
                categories = sorted({cat for _, cat in protected})
                allow = messagebox.askyesno(
                    APP_NAME,
                    "Dupe Demon would like to scan folders macOS treats as "
                    "protected:\n\n"
                    + "\n".join(f"  • {c}" for c in categories)
                    + "\n\nThis reads file names and contents there to look "
                      "for duplicates. Allow it?",
                    icon="question")
                self.prefs.protected_folder_consent = allow
                self.prefs.save()

            if self.prefs.protected_folder_consent:
                return folders, None

            allowed = [e for e, cat in tagged if not cat]
            skipped = len(protected)
            note = (f"Skipped {skipped} protected folder"
                    f"{'s' if skipped != 1 else ''} (access declined) — "
                    "enable in Preferences ▸ Privacy to re-scan them.")
        ...
        def stop_scan(self):
            ...
            if self._render_queue or self._pending_groups:
                ...
                self.progress.stop_spinning()
                self._update_selection_label()

        def _poll_queue(self):
            # A worker thread posts one "progress" message per file, so a
            # large library can queue thousands of them between ticks.
            # Drain everything but only apply the LAST progress/status seen —
            # replaying every message as its own widget .config() call is
            # what made the UI lock up on big libraries (thousands of
            # synchronous redraws before the event loop could handle a click).
            ...
            try:
                while True:
                    ...
                    if kind == "progress":
                        ...
                        latest_text = f"Analyzed {msg[1]} of {msg[2]} images…"
                    elif kind == "status":
                        latest_text = msg[1]
                    elif kind == "done":
                        # a stale progress/status message earlier in this same
                        # batch must not overwrite what the handler sets below
                        latest_progress = latest_text = None
                    ...
                    elif kind == "trash_progress":
                        latest_progress = (msg[1], msg[2])
                        latest_text = f"Moving {msg[1]} of {msg[2]} files to Trash…"
                    elif kind == "trash_done":
                        latest_progress = latest_text = None
                        self._trash_finished(msg[1])
                    elif kind == "trash_error":
                        latest_progress = latest_text = None
                        self._trash_failed(msg[1], msg[2])
            except queue.Empty:
                pass
            ...
            if latest_text is not None:
                self.status_label.config(text=latest_text)
        ...
        def _start_rendering(self, groups, summary_text, track_progress=True):
            """Queue groups for display, one page at a time. Progress tracks
            the TRUE total across every page, not just the current page's
            size — a page boundary (every 40 groups) used to reset the
            pie/status to a new, smaller "of N" that had nothing to do with
            the overall scan, and on a library with hundreds of pages that
            made the pie sit still while the count kept climbing, which
            reads as a hang. track_progress=False keeps the bar as-is (after
            deletes/resorts, overall progress stays put)."""
            ...
            if track_progress:
                self.progress.config(maximum=max(self._groups_total, 1), value=0)
            self.status_label.config(
                text=f"Processing 0 of {self._groups_total} groups…")
            self.stop_btn.state(["!disabled"])
        ...
        def _render_more(self):
            ...
            self.status_label.config(
                text=f"Processing {self._rendered_count} of "
                     f"{self._groups_total} groups…")
            self.stop_btn.state(["!disabled"])
            self._render_queue = page
        ...
        def _render_next_group(self):
            # Render a bounded slice of work per tick so the UI stays
            # responsive. Capping by group count alone isn't enough — a
            # single group can hold many files (burst-mode duplicates), and
            # each thumbnail is a real image decode (HEIC/PNG have no fast
            # "draft" path like JPEG does). A few oversized groups in one
            # tick was blocking the main thread long enough to beachball,
            # and Stop can't be processed by Tk until the tick returns.
            from datetime import datetime
            MAX_GROUPS_PER_TICK = 3
            MAX_THUMBS_PER_TICK = 12
            rendered_groups = 0
            rendered_thumbs = 0
            while self._render_queue:
                if rendered_groups >= MAX_GROUPS_PER_TICK:
                    break
                if rendered_groups > 0 and rendered_thumbs >= MAX_THUMBS_PER_TICK:
                    break
                index, group = self._render_queue.pop(0)
                rendered_groups += 1
                rendered_thumbs += len(group)
                ...
                for fi in group:
                    ...
                    path_label.bind("<Button-1>",
                                    lambda e, p=fi.path: reveal_in_finder(p))
            self._rendered_count += rendered_groups
            if self._render_track_progress:
                self.progress.config(maximum=max(self._groups_total, 1),
                                     value=self._rendered_count)
            if self._render_queue:
                self.status_label.config(
                    text=f"Processing {self._rendered_count} of "
                         f"{self._groups_total} groups…")
            elif self._pending_groups:
                # This page is done but more remain. Wait for the user to
                # actually scroll near the bottom (_check_scroll_load) before
                # loading more — auto-chaining straight into the next page
                # here used to render every group in the whole scan up front
                # regardless of scroll position, which is what made a
                # 7000+ group scan slow: it built real Tk widgets for groups
                # nobody had scrolled to yet. Status says "shown", not
                # "Processing", since nothing is actively happening now.
                self.status_label.config(
                    text=f"{self._rendered_count} of {self._groups_total} "
                         f"groups shown — scroll for more")
            else:
        ...
        def trash_selected(self):
            ...
            if fully_selected:
                ...
                return
            # Finder's AppleScript delete is genuinely slow at scale (each
            # 100-file chunk can take several seconds, and thousands of
            # selected files means dozens of chunks) — running it on the
            # main thread with one static "Moving files…" message for the
            # whole operation looked exactly like a hang for a large
            # selection. Run it in the background instead, like scanning.
            self.trash_btn.state(["disabled"])
            self.move_btn.state(["disabled"])
            self.progress.config(maximum=len(selected), value=0)
            self.status_label.config(
                text=f"Moving 0 of {len(selected)} files to Trash…")

            def worker():
                done = 0
                for start in range(0, len(selected), 100):
                    chunk = selected[start:start + 100]
                    err = move_to_trash(chunk)
                    if err:
                        self._msg_queue.put(
                            ("trash_error", err, list(selected[:done])))
                        return
                    done += len(chunk)
                    self._msg_queue.put(("trash_progress", done, len(selected)))
                self._msg_queue.put(("trash_done", list(selected)))

            threading.Thread(target=worker, daemon=True).start()

        def _trash_finished(self, trashed_paths):
            self.trash_btn.state(["!disabled"])
            self.move_btn.state(["!disabled"])
            trashed = set(trashed_paths)
            total = sum(self.file_by_path[p].size for p in trashed_paths
                       if p in self.file_by_path)
            self._remove_paths_from_results(
                trashed,
                f"Moved {len(trashed_paths)} files ({human_size(total)}) "
                "to the Trash.")

        def _trash_failed(self, error, trashed_so_far):
            applog.error(f"Trash operation failed: {error} "
                        f"({len(trashed_so_far)} file(s) trashed before failure)")
            self.trash_btn.state(["!disabled"])
            self.move_btn.state(["!disabled"])
            # A partial-success re-render below uses track_progress=False
            # (it doesn't touch the pie), so without this the pie freezes
            # mid-spin at whatever partial value the last chunk left it at.
            self.progress.stop_spinning()
            if trashed_so_far:
                # Some chunks succeeded before the failure — keep results in
                # sync with what actually happened rather than losing track.
                self._remove_paths_from_results(
                    set(trashed_so_far),
                    f"Moved {len(trashed_so_far)} files before the error below.")
        ...
        def move_selected_to_folder(self):
            ...
            if failures:
                first = failures[0]
                applog.error(
                    f"move_selected_to_folder: {len(failures)} failure(s) — "
